...
def _same_uv_edge_length(xface: XFace, edge_index: int, other: XFace, other_edge_index: int):
    ...
    return Decimal(edge.length) == Decimal(other_edge.length)
...
def _get_rotation_to_be_aligned(vector_to_check: Vector, vector_to_rotate: Vector):
    for i in range(4):
        ...
        vector_rotated = vector_to_rotate.copy()
        vector_rotated.rotate(rotation_matrix)
        if vector_rotated.normalized().freeze() == vector_to_check.normalized().freeze() \
                or (-vector_rotated).normalized().freeze() == vector_to_check.normalized().freeze():
            return rotation
    return None
...
def _is_the_same_segment(p1: Vector, q1: Vector, p2: Vector, q2: Vector):
    return (p1 == p2 and q1 == q2) or (p1 == q2 and p2 == q1)
    ...
    for i in range(len(points_a)):
        ...
        for j in range(len(points_b)):
            ...
            if segments_intersect(curr_point, next_point, curr_point_b, next_point_b):
                intersections.add(
                    segments_intersection_point(curr_point, next_point, curr_point_b, next_point_b).freeze())
...
def _any_point_inside(points_a: [Vector], points_b: [Vector]) -> bool:
    for point in points_a:
        if point not in points_b and _get_winding_number(point, points_b):
            return True
    return False
    ...
    for i in range(len(points)):
        ...
        if curr_point.y <= point.y:
            if next_point.y > point.y:
                if _is_at_left(curr_point, next_point, point) > 0:
                    winding_number += 1
        else:
            if next_point.y <= point.y:
                if _is_at_left(curr_point, next_point, point) < 0:
